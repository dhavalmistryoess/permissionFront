...
@Component({
  selector: 'app-item-option-changes',
  templateUrl: './item-option-changes.component.html',
  styleUrls: ['./item-option-changes.component.css']
})
export class ItemOptionChangesComponent implements OnInit {
  itemEntity;
  categoryList;
  OptionsList;
  des_valid;
  ...
  submitted1;
  constructor(public globals: Globals,private router: Router,private route: ActivatedRoute,private ItemService: ItemService) {}
    ...
    if (itemForm.valid && !this.des_valid && !this.ItemText_valid && !this.optioncheck_valid &&
      ...
      this.ItemService.addUpdate(this.itemEntity).then(
        data => {
          ...
          if (id) {
            swal({
              type: this.globals.adminTranslationText.item.form.alerts.update
                .type,
              title: this.globals.adminTranslationText.item.form.alerts.update
                .title,
              text: this.globals.adminTranslationText.item.form.alerts.update
                .text,
            ...
            swal({
              ...
              title: this.globals.adminTranslationText.item.form.alerts.add
                .title,
